import os as os
from ..cgsconst import cgsconst as _cgs
from ..helpers import *
import scipy as _sp
from scipy import where, insert, pi
...
def plot_ratraninput(directory = '', modelfile = "transphere.mdl"):
    ...
    Ratran_mdl = read_ratraninput(os.path.join(directory, modelfile))
...
def ratran_environment_check():
    ...
    try:
        ratran_path = os.environ['RATRAN']
    except (KeyError):
        ratran_path = False
    if ratran_path:
        ratran_bin = os.path.join(ratran_path, 'bin')
        global RUN_AMC
...
class Make(object):
    def __init__(self, **kwargs):
        ...
        """
        with open(os.path.join(self.directory, self.modelfile),'w') as f:
        ...
        if not self.skyonly:
            ...
            with open(os.path.join(self.directory, "amc.inp"),'w') as f:
        ...
        with open(os.path.join(self.directory, "sky.inp"),'w') as f:
            if self.skyonly:
                f.write("source={0}\n".format(self.modelfile))
            else:
...
class MakeSky(object):
    ...
    def __init__(self, 
        ...
        #
        # CHECK if directory exists
        input_dir_path = os.path.join(os.getcwd(), directory)
        # if the directory exists
        if not make_dirs(input_dir_path): # will raise error if things go south (i.e., permissions not correct [I think...])
            print('Directory exists, continuing.')
        ...
        # 100 is gas:dust, but the input gas2dust is only for the 
        # run itself, i.e. only related to the molecules
        #~ nh2 = rhodust * 100  / (_cgs.MUH2 * _cgs.MP)
        nh2 = rhodust * gas2dust  / ( muh2 * co.m_p.cgs.value)
        # nh2 is number density of H2
        # rhodust * 100 = rhogas (g/cm3) (all gas H2+He+Metals)
        # MUH2 * MP =  molecular mass (H2+He+Metals) in g
        # so
        ...
        # get values at r = 1000 AU\
        try:
            ind_r1000 = where(r_grid > (1000 * _cgs.AU))[0].min()
            rhodust_r1000 = rhodust[ind_r1000]
            nh2_r1000 =  nh2[ind_r1000]
        ...
        #
        r_grid_m = r_grid * co.au.si.value
        r_grid = insert(r_grid, 0, 0)
        r_grid_m = insert(r_grid_m, 0, 0)
        r1_m = r_grid_m[0:-1]
        r2_m = r_grid_m[1:]
        #~ rr = zeros(len(r_grid)-1, float)
        ...
        #~ V = 4 * pi * ((r2**3  - r1**3 )) / 3     # cm3
        #~ M = V * nh2int * _cgs.MUH2 * _cgs.MP # g = cm3 * g/cm3
        # proper calculation of mass
        # TODO : the density is slightly wrong though, but shouldn't matter
        # too much for these simple models and lines.
        # need to correct this in the future though...
        #~ rho = nh2 * muh2 * co.m_p.cgs.value
        #~ dr = r2 - r1
        #~ r = (r1 + r2)/2.
        #~ M = 4 * pi * _sp.sum(r_grid**2 * rho * dr)
        #~ M /= co.M_sun.cgs.value                            # Msun
        # to get the column density, integrate over radius r1 to r_10k
        #r_10k * 2 nh2_10k
        #~ print M
        ################################################################
        ...
        vr    : radial velocity (km s-1)
        """
        with open(os.path.join(directory, modelfile),'w') as f:
            f.write('# Ratran input file based on Transphere results'+'\n')
            f.write('# ... intended for (SKY) continuum calculations only.'+'\n')
            f.write("rmax={0:.5E}\n".format(r2_m[-1]))       # rmax in METERS
            f.write("ncell={0:}\n".format(len(r2_m)))
            f.write("tcmb=2.735\n")
            #                id,ra,rb,nh,nm,tk,td,db,vr
            f.write("columns=id,ra,rb,nh,td,tk,nm,db,vr\n")
            #~ f.write("columns=id,ra,rb,nh,td\n")
            f.write("gas:dust={0}\n".format(gas2dust))
            f.write("kappa={0}\n".format(kappa))
            f.write('@\n')
            # r1/r2 in meter (convert from cm)
            for ii in range(0, len(r1_m)):
                test = ("{0:4} "                        #  1 id : shell number
                        "{1:12.5E} "                    #  2 ra : inner radius (m)  
                        "{2:12.5E} "                    #  3 rb : outer radius (m)
                        "{3:12.5E} "                    #  4 nh : density (cm-3) of main coll. partner (usually H2)
                        "{4:12.5E}"                     #  5 td : dust temperature (K) 
                        "{5:12.5E}"                     #  6 td : kinetic temperature (K) 
                        "{6:12.5E}"                     #  7 nm : molecular abundance 
                        "{7:12.5E}"                     #  8 db : doppler broadening
                        "{8:12.5E}\n"                   #  9 vr : radial velocity
						)                 
                # now print the whole shebang
                f.write(test.format(ii + 1,             #  1 id : shell number
                    r1_m[ii],                  	        #  2 ra : inner radius (m)  
                    r2_m[ii],                  	        #  3 rb : outer radius (m)
                    nh2[ii],      				 		#  4 nh : density (cm-3) of main coll. partner (usually p-H2)
                    temp[ii],                       	#  5 td : dust temperature (K) 
                    temp[ii],                       	#  6 tk : kinetic temperature (K) 
                    nh2[ii]*1e-5,                 	    #  7 nm : molecular abundance
                    0.8,                       	        #  8 db : doppler broadening
                    0.0)                       	        #  9 vr : radial velocity
                        )
            #~ for ii in range(0, len(r1)):
                #~ test = ("{0:4} "                        #  1 id : shell number
                        ...
                        #~ "{2:12.5E} "                    #  3 rb : outer radius (m)
                        #~ "{3:12.5E} "                    #  4 nh : density (cm-3) of main coll. partner (usually H2)
                        #~ "{4:12.5E}\n"                     #  5 td : dust temperature (K) 
						#~ )                 
                #~ # now print the whole shebang
                #~ f.write(test.format(ii + 1,             #  1 id : shell number
                    #~ r1[ii] / 100.0,                  	#  2 ra : inner radius (m)  
                    #~ r2[ii] / 100.0,                  	#  3 rb : outer radius (m)
                    #~ nh2[ii],      				 		#  4 nh : density (cm-3) of main coll. partner (usually p-H2)
                    #~ temp[ii]                        	#  5 td : dust temperature (K) 
                        #~ ))
                                    
        with open(os.path.join(directory, "sky.inp"),'w') as f:
            f.write("source={0}\n".format(modelfile))
            f.write("format={0}\n".format(outformat))
            f.write("outfile={0}_{1}\n".format(outputfile, unit))
            f.write("outfile={0}_{1}\n".format(outputfile, unit))
            f.write("trans={0:6.3E}\n".format(frequency))
            f.write("pix={0},{1},{2},{3}\n".format(imsize, pixel, pxlradius, los))
            f.write("chan=1,1.0\n")
            f.write("distance={0}\n".format(dpc))
            f.write("units={0}\n".format(unit))
            f.write("go\n")
